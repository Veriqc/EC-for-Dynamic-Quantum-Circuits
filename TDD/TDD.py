...
def single_qubit_state_2_TDD(vector,var):
    ...
    if vector[0]==vector[1]:
        ...
        tdd.weight=vector[0]
        tdd.index_set = var
        tdd.index_2_key={-1:-1,var[0].key:0}
        tdd.key_2_index={-1:-1,0:var[0].key}
        return tdd
    else:
        low=TDD(term)
        low.weight=vector[0]
...
def condition_2_TDD(U,var):
    ...
    for k in range(con_num):
        ...
        if global_index_order[x]>global_index_order[v1]:
            high=change_neig_index(high,2+k,1+k)
        if global_index_order[x]>global_index_order[v2]:
            high=change_neig_index(high,1+k,0+k)
    tdd=high
    tdd.index_set = var
    return tdd
...
def ccx_2_TDD(var):
    ...
    high = if_then_else(x,low,high)
    if global_index_order[x]>global_index_order[v1]:
        high=change_neig_index(high,2,1)
    if global_index_order[x]>global_index_order[v2]:
        high=change_neig_index(high,1,0)
    x=min(var[0],var[3]).key
    high = if_then_else(x,low,high)
    if global_index_order[x]>global_index_order[v1]:
        high=change_neig_index(high,3,2)
    if global_index_order[x]>global_index_order[v2]:
        high=change_neig_index(high,2,1)
    tdd=high
...
def if_then_else(x,tdd1,tdd2):
    k1=max(tdd1.key_2_index)
    k2=max(tdd2.key_2_index)
    k=max(k1,k2)+1
    res = normalize(k,tdd1,tdd2)
    if k2>k1:
        res.index_2_key=tdd2.index_2_key
        res.key_2_index=tdd2.key_2_index
        res.index_2_key[x]=k
        res.key_2_index[k]=x
    else:
        res.index_2_key=tdd1.index_2_key
        res.key_2_index=tdd1.key_2_index
        res.index_2_key[x]=k
        res.key_2_index[k]=x
    return res
...
def cont(tdd1,tdd2):
    ...
    res=contract(tdd1,tdd2,key_2_new_key,cont_order,len(set(var_cont_idx)))
...
def Slicing4(tdd,x,c):
    """Slice a TDD with respect to x=c"""

    k=tdd.node.key
    
    if k<x:
        if x in tdd.key_2_index:
            res=tdd.self_copy()
            the_key=tdd.key_2_index[x]
            for k1 in tdd.key_2_index:
                if k1>=x:
                    res.key_2_index.pop(k1)
                    res.index_2_key.pop(tdd.key_2_index[k1])
            for idx in tdd.index_set:
                if idx<Index(the_key,0) or idx.key==the_key:
                    res.index_set.remove(idx)
            return res
        else:
            tdd.self_copy()
...
def change_neig_index(tdd,x,y):
    """"change the position of x and y, such that x<y in this tdd, it should be x=y+1 original
    """
    v=tdd.node.key
    if v<=y:
        return tdd
    if v>x and v>y:
        low=change_neig_index(Slicing2(tdd,v,0),x,y)
        high=change_neig_index(Slicing2(tdd,v,1),x,y)
        res=normalize(v,low,high)
        res.index_set=copy.copy(tdd.index_set)
        res.index_2_key=copy.copy(tdd.index_2_key)
        res.key_2_index=copy.copy(tdd.key_2_index)
        if res.index_2_key and res.key_2_index:
            res.key_2_index[x],res.key_2_index[y]=tdd.key_2_index[y],tdd.key_2_index[x]
            res.index_2_key[tdd.key_2_index[x]]=y
            res.index_2_key[tdd.key_2_index[y]]=x 
        return res
    if v == x:
        low1 = Slicing2(Slicing2(tdd,x,0),y,0)
        high1 = Slicing2(Slicing2(tdd,x,1),y,0)
        left = normalize(y,low1,high1)
        low2 = Slicing2(Slicing2(tdd,x,0),y,1)
        high2 = Slicing2(Slicing2(tdd,x,1),y,1)
        right = normalize(y,low2,high2)
        res=normalize(x,left,right)
        res.index_set=copy.copy(tdd.index_set)
        res.index_2_key=copy.copy(tdd.index_2_key)
        res.key_2_index=copy.copy(tdd.key_2_index)
        if res.index_2_key and res.key_2_index:
...
def conjugate(tdd):
    ...
    if v==-1:
        ...
        return res
    low=conjugate(Slicing2(tdd,v,0))
    high=conjugate(Slicing2(tdd,v,1))
    res=normalize(v,low,high)
    res.index_set=copy.copy(tdd.index_set)
    res.index_2_key=copy.copy(tdd.index_2_key)
    res.key_2_index=copy.copy(tdd.key_2_index)  
...
def get_norm(tdd):
    ...
    return cont(tdd,tdd_conj).weight
    
def get_norm2(tdd):
    if tdd.node.key==-1 and tdd.weight==0:
        return 0
    if tdd.node.key==-1 and max(tdd.key_2_index)==-1:
        return abs(tdd.weight)**2
    x=max(tdd.key_2_index)
    return get_norm2(Slicing4(tdd,x,0))+get_norm2(Slicing4(tdd,x,1))