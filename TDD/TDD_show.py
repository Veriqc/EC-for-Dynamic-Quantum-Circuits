...
def TDD_show2(tdd):
    ...
    dot.node('-0','',shape='none')
    label1=str(complex(round(tdd.weight.real,2),round(tdd.weight.imag,2)))
#     label1=str(round(tdd.weight.real,2))
    dot.edge('-0',str(tdd.node.idx),color="blue",label=label1)
#     dot.edge('-0',str(tdd.node.idx),color="blue")
    dot.format = 'png'
    return Image(dot.render('output'))
...
def layout2(node,key_2_idx,dot=Digraph(),succ=[]):
    ...
    if node.successor[k]:
        label1=str(complex(round(node.out_weight[k].real,2),round(node.out_weight[k].imag,2)))
#         label1=str(round(node.out_weight[k].real,2))
        if not node.successor[k] in succ:
